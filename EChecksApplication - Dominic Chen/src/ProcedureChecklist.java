...
public class ProcedureChecklist implements ActionListener{
	...
	//constructor
	public ProcedureChecklist(String titleText) {
		
		nameOfAllFiles = MainMenuScreen.getAllFileNames();
		
		this.titleText = titleText;
		wholeTitle = titleText + " - pcd";
	...
	public ProcedureChecklist(String existTitle,String type) {
	...
	private void addLine() {
		...
		if(lineCount == 9) {
			...
			//this will be added to the screen
			JLabel tempLabel = new JLabel();
			tempLabel.setText(inputNewLine.getText());
			...
			tempLabel.setText(lineCount + ". " + text);
			
			//this is only added to the array list
			JLabel tempLabel2 = new JLabel();
			tempLabel2.setText(inputNewLine.getText());
			tempLabel2.setFont(new Font(selectedFont,selectedStyle,30));
			tempLabel2.setForeground(selectedColor);
			
			JCheckBox tempCheckbox = new JCheckBox();
			tempCheckbox.setBounds(1312,245+89*lineCount,15,15);
			
			CheckLine tempCheckLine = new CheckLine(tempLabel2,tempCheckbox,selectedFont,tempStyle,tempColor);
			checkLines.add(tempCheckLine);
		
			procedureBackground.add(tempLabel);
			procedureBackground.add(tempCheckbox);
	...
	private void addLines() {
		...
		for(CheckLine tempLine : checkLines) {
			...
			text = tempLine.getLabel().getText();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==clearButton) {
			...
			if(!checkLines.isEmpty()) {
				...
				new ProcedureChecklist(titleText);
				frame.dispose();
		...
		if(e.getSource()==changeBackground) {
			...
			procedureBackground.setIcon(MainMenuScreen.allBackgrounds[MainMenuScreen.backgroundCount]);
